    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>ten design artifacts</span></div>{documents.map((document, index) => <a key={document.file} className="node-document" href={`https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>)}</section>
          <section className="node-next"><div><span>NEXT PROTECTED-WIRING GATE</span><h2>Compose opaque runtime references locally before any Agent may invoke Core.</h2></div><p>The L4e dossier now fixes the one-shot deployment/proof boundary: no public port, no default runner, Core-only delivery, separate identity, private ephemeral workspace, and aggregate-safe closure. Next, protected runtime wiring may be implemented and locally validated. No deployment, Core/Azure request, training, update, submission, provider contact, or aggregation-worker activation is permitted until release and preflight gates are separately green.</p><CheckCircle2 size={24} /></section>
        </div>
