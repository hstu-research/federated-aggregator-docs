    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>STAGING SOURCE QUALITY</span><h2>Target-staging source is quality-gated and remains inert; the private synthetic image is still target-unbound.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c620747</code> adds an opt-in no-port/no-restart/no-network composition with the status listener disabled and a target-owned opaque image reference; local CI and Hospital Node Quality Gates succeeded. The preceding scalar-only Azure preflight found successful Core liveness/readiness, resolved disabled aggregation, and no Agent candidate. No target secret, Azure configuration, target pull, image binding, Compose render, Agent start, Core call, proof, training, submission, or aggregation action has occurred. A fresh preflight and the distinct target-only pull-credential gate remain required before staging.</p><CheckCircle2 size={24} /></section>
        </div>
