    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — TARGET-LOCAL BUILD PREFLIGHT CLOSED</span><h2>The target can host a private build workspace, but it lacks the declared native Node build chain.</h2></div><p>The owner-designated target-local build path now binds one exact validated Agent revision to a one-way integrity-checked transfer, private workspace, scalar receipt, and terminal closure. A read-only check found candidate capacity and no Agent/aggregation process, but no Node/Corepack/pnpm toolchain; the result is <code>target_local_build_preflight_closed</code>. A container client is not a substitute without a separately reviewed immutable build-image boundary. No source transfer, tool/dependency install, image build, container/Agent start, Core contact, proof, data/model/trainer, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
