    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>The secret-source boundary is designed; no real identity, Agent target, or Azure topology may be staged.</h2></div><p>Agent and Core source gates are green: all four Agent ports now compose through deterministic fakes only, while the Core retains a deterministic coordinator chain and source-only runner. The new secret-source design permits no secret read or token acquisition; its first executable slice remains fake-only. Azure read-only checks still confirm Core live/ready health and the aggregation worker disabled marker, but the target has no Hospital Node Agent source, built/released Agent image, concrete identity/HTTP/filesystem/channel adapters, executable Core coordinator runner, or composite profile to render. The proof remains blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
