    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PERSISTENCE/RESTART DESIGN</span><h2>The quality-gated scalar diagnostic now has an additive restart-suppression design; external organization authorization remains unavailable.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>63359ba</code> implements a pure validator and deterministic fake, with no provider/runtime imports. The next contract permits only version, bounded sequence, blocked state, allowlisted terminal code, replay-suppressed state, and retry disabled; it excludes identifiers, account/package/credential/SSO/target/provider/path/free-text facts. Any reloaded closure conservatively blocks later evaluations before a fake runs. This is source-only persistence/restart design—not a durable store, permission proof, or staging result. No external capability has been invoked.</p><CheckCircle2 size={24} /></section>
        </div>
