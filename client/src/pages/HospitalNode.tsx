    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>fd43813</i></strong></div><p>Hospital Node Quality Gates run 32668003652 completed successfully after the deterministic fake secret-material and identity-exchange release: formatting, strict TypeScript, 65 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>The protected secret-read edge is designed; no projection may be opened and no Agent target or Azure topology may be staged.</h2></div><p>Agent and Core source gates are green: all four Agent ports and fake identity ports compose through deterministic tests only, while the Core retains a deterministic coordinator chain and source-only runner. The protected secret-read design permits no Node filesystem import, projection open, secret read, or token acquisition; its first executable slice remains an injected fake metadata/read/closure port. Azure read-only checks still confirm Core live/ready health and the aggregation worker disabled marker, but the target has no Hospital Node Agent source, built/released Agent image, concrete secret-read/identity/HTTP/filesystem/channel adapters, executable Core coordinator runner, or composite profile to render. The proof remains blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
