    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Source-only execution readiness is validated; the Agent remains image-free, target-unbound, and blocked from staging or projection access.</h2></div><p>The decision separates policy, execution-approval, registry-release, and credential-custodian authority. The source-only readiness record now validates immutable admission, expiry, and terminal four-fake closure; ready-not-authorized is explicitly not an execution permit. It creates and uses no credential. It authorizes no build daemon, registry contact, image push, Azure staging, Compose profile, target binding, projection, or runner. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work is a separate documentation-only external execution-authorization decision; actual execution remains separately blocked. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
