    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>SCRIPTED-PORT COMPOSITION — QUALITY GATED</span><h2>A pure composition seam now exercises ownership and closure with scalar scripts—never a target-root factory.</h2></div><p>Agent revision <code>923b8eb</code> adds the source-only composition. It accepts one frozen exact scalar script, privately builds one in-memory scalar port, injects it into one fake coordinator, consumes once, and returns only a frozen assertion. Normal and hydration/claim/route/terminal/disposal/replay closures, malformed input before construction, isolation, redaction, and import separation are tested. The full local chain passed 182 TypeScript plus 4 Python tests; remote Quality Gates run <code>32725658219</code> succeeded. This is source quality only: no temporary-root factory call, application wiring, coordinator runtime, target root, filesystem/environment/network action, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
