    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>SYNTHETIC INJECTION CONTRACT</span><h2>Private-root injection is now specified as a pure synthetic nonenumerable contract; target controls remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c3353e6</code> provides source-only configuration policy. The next contract accepts only fixed-format non-path sentinels, scalar eligible/ready-not-activated state, one unused injection identifier, and a deterministic construction fake. Sentinels are private/nonenumerable and cannot enter serialization, inspection, own-property enumeration, errors, status, or public records. Malformed/noneligible/fallback/duplicate/construction-failure cases close before retry or alternate root. No raw reference/parser/mount/build/pull/target/credential/proof/training/submission/aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
