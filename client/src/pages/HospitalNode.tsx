    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>STAGING PRE-ROUTE BLOCK</span><h2>Target-staging source is quality-gated, but no target-only private-package pull credential exists; the synthetic image remains unbound.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c620747</code> adds an opt-in no-port/no-restart/no-network composition with the status listener disabled; local CI and Hospital Node Quality Gates succeeded. A presence-only Azure check found no private-registry login and read no credential material. No target secret, human/browser/repository-job credential reuse, package query, image pull/binding, Compose render, Agent start, Core call, proof, training, submission, or aggregation action occurred. A newly provisioned target-only package-read credential and a fresh Core/aggregation preflight are required before one staging attempt.</p><CheckCircle2 size={24} /></section>
        </div>
