    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>041c386</i></strong></div><p>Hospital Node Quality Gates run 32668418777 completed successfully after the deterministic fake protected-projection lifecycle release: formatting, strict TypeScript, 68 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>The Node secret-read edge is designed; no filesystem import or projection open may occur and no Agent target or Azure topology may be staged.</h2></div><p>Agent and Core source gates are green: all four Agent ports and fake identity/projection ports compose through deterministic tests only, while the Core retains a deterministic coordinator chain and source-only runner. The Node filesystem review permits no source import, projection open, secret read, or token acquisition; its next executable slice remains an injected fake descriptor-first syscall port. Azure read-only checks still confirm Core live/ready health and the aggregation worker disabled marker, but the target has no Hospital Node Agent source, built/released Agent image, concrete secret-read/identity/HTTP/filesystem/channel adapters, executable Core coordinator runner, or composite profile to render. The proof remains blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
