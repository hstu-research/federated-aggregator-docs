    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>TARGET-STAGING PREFLIGHT</span><h2>Azure Core liveness/readiness and disabled aggregation are now read-only verified; the private synthetic image remains target-unbound.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. A scalar-only Azure inspection found one labelled Core API and one labelled aggregation worker, successful internal liveness/readiness, resolved disabled aggregation, and no Agent target candidate. Package inventory cannot be listed by the available integration, so no artifact identity or visibility is publicly claimed. No target secret, Azure configuration, target pull, image binding, Compose render, Agent start, Core call, proof, training, submission, or aggregation action has occurred. Static target-composition source is the next isolated slice; preflight must be repeated before staging.</p><CheckCircle2 size={24} /></section>
        </div>
