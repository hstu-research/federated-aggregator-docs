    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DURABLE-STORE COMPOSITION REVIEW</span><h2>A future injected port can persist scalar coordinator closure without exposing root, records, or artifacts.</h2></div><p>The coordinator terminal-record design now defines its composition seam: an injected port hydrates opaque private correlation into scalar closure, appends one claim, then appends one terminal category. Only the local-state composition root may own a private-root capability; the coordinator sees no root, record, path, identity value, text, or byte count. Hydrate suppresses terminal/orphan states, claim failure blocks routing, and terminal append failure closes without replay. No store is wired, no root/record is created, and no parser, renderer, configuration text/file, target action, package/credential access, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
