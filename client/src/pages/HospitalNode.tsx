    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>CONFIGURATION INTENT CONTRACT</span><h2>Private-root configuration and fresh-image eligibility are now specified as a pure scalar contract; target controls remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>e3070f3</code> is source-only composition policy. The next validator accepts only scalar mode/binding/source-release/image-binding/activation facts and emits terminal disabled or blocked, retry-disabled outcomes. Its disabled state is exactly disabled/no-binding/no-selected-image; private intent denies missing root, unverified/stale/mutable/unavailable image, active/default runtime, malformed/fallback, and replay claims before deterministic fakes. It cannot receive a path, mount, environment name, image/package/digest/registry value, target, credential, provider/process/listener fact. No parser/configuration/build/pull/target/proof/training/submission/aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
