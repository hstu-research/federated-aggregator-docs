    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — TARGET-BOUND RELEASE IDENTITY DESIGNED</span><h2>A one-way release binding is specified, but no source, seed, target, or authority packet can admit it yet.</h2></div><p>The contract separates release authority from source, provision, and target runtime; it binds one immutable source-quality class, toolchain/seed class, and opaque target identity to one expiry-bounded intent, verification, and closure. The current state is <code>target_release_identity_design_only</code>: provenance exception, target-admissible seed, protected composition, fresh Core control, release authority, and target identity are unavailable. No release, target contact, transfer, installation, build, runtime, proof, data/model/trainer, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
