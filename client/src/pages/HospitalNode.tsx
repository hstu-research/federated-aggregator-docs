    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DURABLE-STORE DESIGN</span><h2>The scalar diagnostic has a private durable-store adapter design; implementation and external organization authorization remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>4c7d352</code> provides only in-memory terminal persistence. The new durable-store design confines filesystem access to a future infrastructure adapter and gives the composition root sole ownership of one prevalidated private state-root capability. It defines one fixed canonical scalar record, strict permission/symlink/type checks, bounded read, atomic promotion, corruption closure, private temporary cleanup, and aggregate-only observability. No directory, file, database, configuration, credential, package, target, proof, training, submission, or aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
