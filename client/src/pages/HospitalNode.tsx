    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>A source-only Node secret-read edge is authorized; image binding, Azure staging, and any projection open remain blocked.</h2></div><p>The owner authorized one isolated, fail-closed Node edge and injected syscall-double tests for the Azure test target. It may not become a generic filesystem capability or acquire a token; the source slice has no image, deployment, Compose, provider, or runtime authority. Agent and Core gates remain source-only, while Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker. The target still has no Hospital Node Agent source, built/released Agent image, concrete target binding, executable Core coordinator runner, or composite profile to render. The proof remains blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
