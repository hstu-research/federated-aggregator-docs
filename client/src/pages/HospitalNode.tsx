    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>CONFIGURATION & IMAGE-MAPPING DESIGN</span><h2>Private-root configuration and fresh image mapping are designed as separate target-only controls; deployment and external organization authorization remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>e3070f3</code> is source-only composition policy. The new design separates a scalar-safe intent parser from a target-only private root injector that cannot cross into application/status/log/public surfaces. It accepts only mode/binding-presence/source-release/image-binding/disabled-activation classes. The existing image candidate predates these sources and is ineligible; a fresh immutable candidate is required later. Static rendering remains disabled/no-port/no-ingress/no-restart. No parser/configuration/image/package/target/credential/proof/training/submission/aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
