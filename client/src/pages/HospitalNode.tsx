    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>COMPOSITION OWNERSHIP — DESIGN ONLY</span><h2>One future test-composition seam may join a port and fake coordinator, without becoming an application runtime.</h2></div><p>The next review makes a future test-composition module the sole private owner of one temporary-root factory input, correlation, scalar port, and fake coordinator. It may inject only the five-operation port, admit one no-artifact flow, collect a frozen aggregate-only assertion, dispose once if still open, and discard both objects. Construction failure, denial, disposal, replay, and fixture exception close with no replacement port or retry. It accepts and returns no root, correlation, record, path, text, configuration, target, credential, transport, identity, stateful object, or capability. No application wiring, coordinator runtime, target root, filesystem/environment/network action, parser, renderer, configuration, package/credential, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
