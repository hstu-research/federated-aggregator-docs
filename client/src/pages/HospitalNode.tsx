    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>CREDENTIAL INCREMENT AUTHORIZED</span><h2>The target-only private-package credential increment is authorized; the synthetic image remains unbound until custody and fresh preflight pass.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c620747</code> adds an opt-in no-port/no-restart/no-network composition with the status listener disabled; local CI and Hospital Node Quality Gates succeeded. A presence-only Azure check found no private-registry login and read no credential material. The newly authorized increment permits only a narrow Azure-target package-read identity, never user/browser/repository operator, Core, ML-worker, callback, public, or package-wide credential reuse. No package query, image pull/binding, Compose render, Agent start, Core call, proof, training, submission, or aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
