    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIAGNOSTIC SOURCE QUALITY</span><h2>The deterministic scalar authorization diagnostic is quality-gated; external organization authorization remains unavailable.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>63359ba</code> implements a pure versioned validator and deterministic fake: it accepts only allowlisted scalar states, rejects malformed/unknown input before a fake, maps each state to terminal blocked/no-retry, and denies fallback identity plus replay. Local CI and Hospital Node Quality Gates succeeded, with a source guard prohibiting provider/runtime imports. No provider, organization, package, credential, machine account, target, image, Azure, render, Agent start, Core call, proof, training, submission, or aggregation action occurred. The next low-risk slice is source-only persistence/restart design, not another external attempt.</p><CheckCircle2 size={24} /></section>
        </div>
