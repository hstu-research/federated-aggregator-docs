    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — INDEPENDENT EXACT ACQUISITION DESIGNED</span><h2>The closed Corepack route is not retried: one isolated npm-client route now guards the same exact pnpm class.</h2></div><p>The alternate local client supports exact-version requests. Its one-use route accepts only <code>pnpm@10.16.1</code>, scripts-disabled transient custody, exact manifest/integrity/size/provenance checks, scalar readout, cleanup, and terminal closure; it rejects Corepack reuse, mutable selectors, mirrors, shared cache, activation, target transfer, and runtime broadening. No npm resolution/download/inspection/verification, seed, target, source build, container/Agent, Core/runtime, proof, data/model/trainer, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
