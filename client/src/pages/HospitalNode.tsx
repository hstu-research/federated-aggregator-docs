    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>TERMINAL CREDENTIAL BLOCK</span><h2>The target-only credential attempt was denied by the private registry and revoked; the synthetic image remains unbound.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c620747</code> adds an opt-in no-port/no-restart/no-network composition with the status listener disabled; local CI and Hospital Node Quality Gates succeeded. A new time-limited package-read credential was attempted after fine-grained package-read access proved unavailable, but registry authentication was denied before any image selection or pull. A credential-observability failure was contained by immediate issuer-side revocation; target registry custody remains absent and the session is closed. No package query, image pull/binding, Compose render, Agent start, Core call, proof, training, submission, or aggregation action occurred. A separate package-access diagnosis and alternative target-only identity path are required.</p><CheckCircle2 size={24} /></section>
        </div>
