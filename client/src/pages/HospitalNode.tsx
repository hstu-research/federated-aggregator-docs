    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — DURABLE ADAPTER QUALITY GATED</span><h2>The direct path now has a hardened local durable boundary—and a clear, blocked route to runtime proof.</h2></div><p>Agent revision <code>6d1235f</code> hardens the temporary-test-root claim-terminal adapter: canonical root-entry allowlisting, complete-write handling, and terminal closure for residue, unknown material, symbolic/non-private roots, record symlinks, malformed/oversized/truncated records, interrupted state, duplicate transitions, and replay. The full local chain passed 197 TypeScript plus 4 Python tests; remote Quality Gates run <code>32733992659</code> succeeded. The next runtime gate remains blocked by separate package/identity, target-bound release, protected composition, non-public invocation, and fresh Core-control evidence. No Agent was started; no proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
