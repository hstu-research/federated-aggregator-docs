    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>8d3c415</i></strong></div><p>Hospital Node Quality Gates run 32690774152 completed successfully after the source-only protected-builder execution-readiness release: formatting, protected filesystem, builder-orchestration, and readiness import guards, strict TypeScript, 86 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>The one-candidate execution envelope is documented; the Agent remains image-free, target-unbound, and blocked from staging or projection access.</h2></div><p>The decision requires immutable candidate facts, independent policy/execution/registry/custody reviews, bounded freshness, redacted audit, terminal denial/revocation/quarantine, post-route failure publication, and renewed no-target-deployment separation. Its only ready state is non-executable: it appoints no operator, identifies no candidate, and creates or uses no credential. It authorizes no build daemon, registry contact, image push, Azure staging, Compose profile, target binding, projection, or runner. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work may model this authorization envelope with source-only fakes; actual execution remains separately blocked. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
