    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>FAKE-ONLY RENDERER SOURCE QUALITY</span><h2>Blueprint and workspace fakes are quality-gated; no render text or artifact was created.</h2></div><p>Release <code>f829bfd</code> implements the fake-only renderer orchestrator, private nonenumerable synthetic blueprint reference, scalar blueprint fake, symbolic workspace fake, frozen readout, and import guard. One inert candidate can reach only <code>not_rendered_after_discard</code>; malformed, fallback, unavailable, invalid, temporary, validation, cleanup, and replay cases close terminally. Local CI and Hospital Node Quality Gates passed. No renderer, configuration text/file, filesystem, environment, image/root binding, package/credential/target, staging, proof, training, submission, or aggregation action occurred. Concrete renderer implementation remains separately gated.</p><CheckCircle2 size={24} /></section>
        </div>
