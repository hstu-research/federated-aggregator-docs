    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>b50fdc9</i></strong></div><p>Hospital Node Quality Gates run 32691244949 completed successfully after the source-only one-candidate execution-authorization envelope release: formatting, all protected import guards, strict TypeScript, 91 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>The one-candidate execution-action decision is documented; the Agent remains image-free, target-unbound, and blocked from staging or projection access.</h2></div><p>The decision defines non-transferable authorization consumption, independent freshness rechecks, credential-custody handoff prohibition, redacted route declaration, pre/post-route closure, candidate retirement, no retry, and renewed target separation. Its route-started state is a source-only placeholder that asserts no external action. It consumes no envelope, appoints no candidate, and creates or uses no credential. It authorizes no build daemon, registry contact, image push, Azure staging, Compose profile, target binding, projection, or runner. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work may model one-candidate action consumption with source-only fakes; actual execution remains separately blocked. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
