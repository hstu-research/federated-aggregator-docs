    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Bind reviewed Core and Agent runners into the protected Azure topology before a one-shot profile can create its handoff.</h2></div><p>Agent and Core source gates are green: the Agent has a fake-tested scalar file-channel adapter and non-executing directional topology template, while the Core has a deterministic coordinator channel fake with passing protected deployment. Core is live/ready and its aggregation worker remains disabled. The target still has no deployed Agent image, concrete Core coordinator runner, or reviewed composite profile, so the proof is blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
