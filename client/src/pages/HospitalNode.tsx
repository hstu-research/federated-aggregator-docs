    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>AUTHORIZATION DIAGNOSTIC BLOCK</span><h2>The dedicated machine-identity design is public, but scalar organization authorization is unavailable to the current integration.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c620747</code> retains an inert no-port/no-restart/no-network composition. The available integration was asked only for scalar organization-membership state and received an access denial; it did not read package/image/visibility/linkage/inheritance/SSO/member/credential/target facts. The denial cannot distinguish missing privilege from integration limitation, so no identity switch, browser-role substitution, package query, or credential retry is allowed. The next safe slice is source-only diagnostic contracts and deterministic denials; a real machine-account provisioning gate remains blocked pending separately evidenced administrator authorization.</p><CheckCircle2 size={24} /></section>
        </div>
