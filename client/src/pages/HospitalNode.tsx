    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>SYNTHETIC ADMISSION COORDINATOR SOURCE QUALITY</span><h2>Matched scalar identities are quality-gated through validator-first admission and symbolic discard.</h2></div><p>Release <code>26b471b</code> implements the source-only coordinator, private validator/policy ports, operation replay set, frozen readout, integration fixtures, and import guard. It validates matched operation/shape/render identities, calls validator first, reaches policy only after <code>inert_shape</code>, and yields <code>not_rendered_after_discard</code> only after symbolic discard. Malformed/cross-identity inputs call neither component; validator denial reaches no policy/workspace. Local CI and Hospital Node Quality Gates passed. No parser, renderer, configuration text/file, filesystem, target action, package/credential access, staging, proof, training, submission, or aggregation action occurred. Coordinator terminal persistence remains separately gated.</p><CheckCircle2 size={24} /></section>
        </div>
