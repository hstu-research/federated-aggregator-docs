    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>External builder readiness is documented; the Agent remains image-free, target-unbound, and blocked from staging or projection access.</h2></div><p>The decision separates policy, execution-approval, registry-release, and credential-custodian authority; requires one-candidate immutable evidence, expiry/revocation, redacted audit, terminal quarantine, isolated non-root builder constraints, pin re-verification, and no-target-deployment re-attestation. It appoints no operator and creates or uses no credential. It authorizes no build daemon, registry contact, image push, Azure staging, Compose profile, target binding, projection, or runner. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work may define source-only execution-readiness contracts with fakes; actual execution remains separately blocked. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
