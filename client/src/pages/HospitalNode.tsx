    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>SOURCE-ONLY DIAGNOSTIC CONTRACT</span><h2>Organization authorization remains unavailable to the current integration; a deterministic scalar contract is defined before any identity action.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c620747</code> retains an inert no-port/no-restart/no-network composition. The available integration was asked only for scalar organization-membership state and received an access denial; it did not read package/image/visibility/linkage/inheritance/SSO/member/credential/target facts. The new contract accepts only allowlisted scalar states and emits terminal blocked/no-retry outcomes, denying unknown input, fallback identity, and replay before a deterministic fake. It has no provider, configuration, network, browser, package, credential, Azure, target, proof, training, submission, or aggregation capability.</p><CheckCircle2 size={24} /></section>
        </div>
