    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — TARGET COMPOSITION CLOSED</span><h2>The target has private workspace posture, but no native toolchain and no current admission packet set.</h2></div><p>A single read-only SSH review observed an eligible private workspace and cleanup posture, with no Agent, aggregation-related process, or corresponding application listener. Its native Node/Corepack/pnpm class is absent, so <code>target_composition_closed</code> blocks seed transfer, installation, source build, and activation. This is point-in-time posture only; it does not cure the separate provenance-exception, source/release, or Core-control blocks. No configuration/credential/log/data read, resource change, service, Agent/Core/runtime, proof, trainer, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
