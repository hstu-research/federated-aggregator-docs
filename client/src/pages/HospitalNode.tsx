    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>seventeen design artifacts</span></div>{documents.map((document, index) => <a key={document.file} className="node-document" href={`https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>)}</section>
          <section className="node-next"><div><span>DIRECT THESIS PATH — RELATED-WORK OUTLINE PUBLISHED</span><h2>The thesis can position FedProx without borrowing an empirical result.</h2></div><p>The new outline uses FedProx, healthcare federated-learning, and governance literature only as context for a narrow source-only contribution. It explicitly excludes claims of new optimization, model performance, clinical application, privacy outcome, or institutional governance effectiveness. No remote, target, study, runtime, or clinical activity is authorized or evidenced.</p><CheckCircle2 size={24} /></section>
        </div>
