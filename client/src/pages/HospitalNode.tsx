    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>COMPOSITION CONTRACT SOURCE QUALITY</span><h2>Private-root composition is quality-gated as a pure scalar policy; deployment and external organization authorization remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>e3070f3</code> implements the versioned private-root composition validator and deterministic fake. It maps root absence/invalidity/temporary artifacts, unbound binding, target-runtime claims, destructive rollback, fallback roots, default activation, malformed observations, and replay to terminal retry-disabled outcomes. Only ready-not-activated and binding-removed rollback are disabled, not activated. Local CI and Hospital Node Quality Gates succeeded. No filesystem/environment/deployment/root/mount/target/Azure/provider/credential/package/image/Agent/Core/proof/training/submission/aggregation action occurred. A real composition/configuration adapter remains a separate boundary.</p><CheckCircle2 size={24} /></section>
        </div>
