    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>ffc588b</i></strong></div><p>Hospital Node Quality Gates run 32696009922 completed successfully after the real manual protected-builder workflow release: formatting, all protected import guards, strict TypeScript, 104 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>TERMINAL BUILD CLOSURE</span><h2>The first private live candidate is closed on unavailable GitHub attestation storage; no retry or target route is permitted.</h2></div><p>The real checkout, build platform, private registry authentication, and build/publish step completed, but GitHub provenance-attestation persistence is unavailable under the current private-repository capability. The candidate is terminally closed and package inventory cannot be independently inspected by the available integration, so no public artifact claim is made. No retry, registry fallback, mutable tag, Azure action, target binding, Compose render, projection, proof, training, submission, or aggregation action occurred. The next isolated step is a fresh source revision using fixed BuildKit OCI provenance, followed by one fresh candidate only after quality evidence.</p><CheckCircle2 size={24} /></section>
        </div>
