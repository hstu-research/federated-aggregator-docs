    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Read-only environment feasibility is blocked; the Agent remains image-free, target-unbound, and blocked from provider configuration or external execution.</h2></div><p>Read-only repository inspection found a private active repository, read-only default workflow-token posture, automatic pull-request approval disabled, and zero configured environments. That does not evidence an environment feature, independent reviewer/custodian assignment, no-bypass control, branch restriction, protected secret boundary, or private package policy. The feasibility result is blocked, and no provider resource was changed. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next boundary requires an explicitly authorized protected-environment/private-package configuration plan with independent roles; every configuration and external execution action remains security-critical. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
