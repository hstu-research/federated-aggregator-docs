    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>LIVE-TEST BUILD GATE</span><h2>The real manual builder is configured and quality-gated; one private synthetic image build is the next isolated action.</h2></div><p>The real test environment has no secrets, no branch policy, and zero protection rules under the explicitly reduced single-admin exception. The dedicated workflow is manual-only, false-by-default, main-guarded, single-candidate, cancellation-free, time-bounded, SHA-pinned, minimally permissioned, and provenance-capable. It has not run: no package, image, registry publication, Azure action, target binding, Compose render, projection, proof, training, submission, or aggregation action exists. The next isolated action is exactly one manual private synthetic build; Azure staging and proof remain blocked behind later records.</p><CheckCircle2 size={24} /></section>
        </div>
