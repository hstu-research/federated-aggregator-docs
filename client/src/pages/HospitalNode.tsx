    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — LOCAL SEED PREFLIGHT CLOSED</span><h2>The required Node build contract is exact: a different local pnpm client cannot become the seed.</h2></div><p>The local environment has Node 22, Corepack, and a pnpm client, but not the Agent-declared <code>pnpm@10.16.1</code> class. Accepting a substitute would violate the immutable toolchain boundary, so the result is <code>toolchain_seed_preflight_closed</code>. No package resolution/download/activation, seed creation, transfer, target change, source build, container/Agent start, Core contact, proof, data/model/trainer, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
