    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>eighteen design artifacts</span></div>{documents.map((document, index) => <a key={document.file} className="node-document" href={`https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>)}</section>
          <section className="node-next"><div><span>DIRECT THESIS PATH — TERMINAL READINESS CLOSURE RESTORES IN MEMORY ONLY</span><h2>A restart simulation cannot reopen an ineligible evaluation state.</h2></div><p>The source-only terminal ledger retains one fixed scalar closure from the all-gates-unassigned receipt. A canonical in-memory restart snapshot restores that closure and suppresses later consumption; unsafe input terminally closes. It is not external persistence and cannot issue authority, approval, or a study path. Every independent readiness gate remains unsatisfied; no data/model, training, metric, aggregation, remote, target, runtime, hospital, clinical, privacy, or performance evidence exists.</p><CheckCircle2 size={24} /></section>
        </div>
