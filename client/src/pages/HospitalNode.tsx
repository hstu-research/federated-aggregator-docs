    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>COORDINATOR TERMINAL-RECORD DESIGN</span><h2>A future private ledger can claim once, close once, and suppress restarts without reviving work.</h2></div><p>The synthetic coordinator remains quality-gated at <code>26b471b</code>. The new review defines a private append-only scalar ledger: write an opaque <code>claimed</code> state before coordinator invocation, then one terminal state afterward. Hydration suppresses valid terminal records; an orphan claim becomes closed-interrupted, never resumed. Corruption, permission, temporary, atomic, symlink, duplicate, and non-monotonic failures close before routing. Public readout remains aggregate-only. No durable store is wired, no root/record is created, and no parser, renderer, configuration text/file, target action, package/credential access, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
