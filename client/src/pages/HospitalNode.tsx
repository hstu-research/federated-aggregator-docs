    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>fifteen design artifacts</span></div>{documents.map((document, index) => <a key={document.file} className="node-document" href={`https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>)}</section>
          <section className="node-next"><div><span>DIRECT THESIS PATH — TRACEABILITY APPENDIX PUBLISHED</span><h2>Every thesis statement can now be checked against its evidence boundary.</h2></div><p>The appendix maps the research question and source-only controls to supportable wording, while marking empirical, clinical, hospital, and runtime evidence as absent. It helps prevent a local contract result from being presented as a machine-learning or clinical finding. No remote, target, study, runtime, or clinical activity is authorized or evidenced.</p><CheckCircle2 size={24} /></section>
        </div>
