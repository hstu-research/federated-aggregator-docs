    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRIVATE-ROOT COMPOSITION DESIGN</span><h2>Durable state is now assigned a private composition/rollback boundary; deployment and external organization authorization remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>8ad309f</code> implements only local durable storage. The new design gives the target operator root provisioning, deployment configuration opaque binding, infrastructure composition validation/adapter construction, and application only the persistence port. Composition is opt-in and disabled by default; the existing image is not repurposed. It defines scalar preflight, root denial without fallback, aggregate-only observability, and conservative final-record-preserving rollback. No root/configuration/mount/image/Azure/Agent/package/credential/Core/proof/training/submission/aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
