    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PERSISTENCE SOURCE QUALITY</span><h2>The scalar diagnostic now has quality-gated in-memory terminal persistence and restart suppression; external organization authorization remains unavailable.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>4c7d352</code> implements a pure terminal-record coordinator, restart-safe wrapper, strict scalar validator, and deterministic in-memory fake. Its record accepts only version, bounded sequence, blocked state, allowlisted terminal code, replay-suppressed state, and retry disabled; malformed, unknown, duplicate, and forbidden-shaped records close. A reloaded closure or append fault suppresses later evaluator work. Local CI and Hospital Node Quality Gates succeeded. This is not a durable store or external authorization proof: no filesystem, database, provider, credential, package, target, proof, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
