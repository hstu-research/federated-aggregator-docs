    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>8052024</i></strong></div><p>Hospital Node Quality Gates run 32690357676 completed successfully after the source-only protected-builder admission-orchestration release: formatting, protected filesystem and builder-import guards, strict TypeScript, 81 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Source-only builder orchestration is validated; the Agent remains image-free, target-unbound, and blocked from staging or projection access.</h2></div><p>The decision separates policy authority, a future protected builder, and a registry-release approver. The source-only orchestrator now validates immutable admission facts and returns only terminal scalar authorization, denial, or quarantine through deterministic fakes; no result is an execution permission. It records no credential creation or use and requires a no-target-deployment attestation before any later execution approval. It authorizes no build daemon, registry contact, image push, Azure staging, Compose profile, target binding, projection, or runner. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work is a separate documentation-only external builder execution/credential-custody decision; any protected build remains blocked. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
