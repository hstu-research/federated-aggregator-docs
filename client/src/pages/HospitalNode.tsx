    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — SSH PREFLIGHT CLOSED</span><h2>The target is reachable, but all package, release, composition, and Core-control evidence remains unverified.</h2></div><p>One read-only SSH inspection observed only scalar posture: no Agent or aggregation-related process was found. The target-only package-read attestation, immutable release binding, protected composition/configuration, and fresh Core-control preflight were absent, so the result closes as <code>target_access_preflight_closed</code>. No configuration, credential, package content, log, or application data was read; no resource changed; and no package request, Agent start, Core contact, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
