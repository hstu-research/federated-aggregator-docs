    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Protected-environment policy is documented; the Agent remains image-free, target-unbound, and blocked from external execution or staging.</h2></div><p>The selected builder now has a documentation-only manual-review policy: repository feasibility must be verified before configuration; a separate environment requires independent non-self review, no bypass, one-candidate freshness, minimal ephemeral permissions, SHA-pinned actions, redacted audit, cancellation, revocation, and target separation. It does not create an environment, workflow, package, credential, image, or registry route. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work is a source-only workflow-policy validator; all provider configuration and external execution remain security-critical gates. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
