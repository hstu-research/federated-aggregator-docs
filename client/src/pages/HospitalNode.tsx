    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Source-only workflow-policy validation is complete; the Agent remains image-free, target-unbound, and blocked from provider configuration or external execution.</h2></div><p>The policy validator now accepts only fixed manual activation, non-self review/no bypass, private package posture, minimal permissions, SHA pinning, single-candidate/no-cancellation, bounded timeout, and target-unbound classes through deterministic fakes. A valid result is explicitly source-only and cannot approve an environment, workflow, credential, package, image, registry route, or target action. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work is an evidence-only environment-feasibility record; every provider configuration and external execution step remains security-critical. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
