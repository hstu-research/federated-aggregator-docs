    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — NATIVE TOOLCHAIN DESIGN ONLY</span><h2>The target-native build now has one fixed toolchain contract, but no toolchain seed has been provisioned.</h2></div><p>The Agent declares <code>pnpm@10.16.1</code> and a Node 22 compatibility class. The target remains without Node/Corepack/pnpm, so the path requires a one-use integrity-checked native seed in a private workspace—not a system install, mutable provider resolution, or unreviewed container image. The current result is <code>toolchain_unavailable</code>. No toolchain download/install/transfer, source build, container/Agent start, Core contact, proof, data/model/trainer, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
