    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>NEXT LOCAL VERIFICATION GATE</span><h2>Exercise a fake workspace before an Agent is allowed to call Core.</h2></div><p>The Agent now has scalar-safe receipt contracts, terminal receipt state rules, and additive SQLite evidence—without a Core connection or byte handling. The next local-only use case may hash generated bytes into a fake temporary workspace, prove mismatch cleanup and restart behavior, and stop before any OIDC token, Core HTTP stream, Azure Agent execution, Python training, update, submission, provider contact, or aggregation-worker activation.</p><CheckCircle2 size={24} /></section>
        </div>
        <aside className="node-provenance" aria-label="Bounded Azure proof provenance">
          <span>PROOF / 004</span>
          <h2>Local receipt<br />evidence</h2>
          <StatusStamp status="VALIDATED" />
          <div><span>RELEASE</span><strong>a9561ff</strong></div>
          <div><span>AUTHORITY</span><strong>local SQLite</strong></div>
          <div><span>BOUND</span><strong>scalar receipt facts</strong></div>
          <div><span>CLOSURE</span><strong>terminal / restart-safe</strong></div>
          <p>Local contract and persistence only. No Core connection, bytes, workspace, training, submission, hospital data, or provider access.</p>
        </aside>
      </div>
    </div>
  );
