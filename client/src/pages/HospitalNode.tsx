    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>The protected image-build decision is complete; the Agent remains image-free, target-unbound, and blocked from staging or projection access.</h2></div><p>The decision requires immutable source/dependency/runtime/policy admission, non-root/no-listener/no-proof-default constraints, a production-source import guard, digest-only release mapping, redacted build outcomes, and quarantine. It authorizes no build daemon, registry contact, image push, Azure staging, Compose profile, target binding, projection, or runner. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. The next safe work is source-only admission/mapping validation; any protected build remains a separately authorized later gate. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
