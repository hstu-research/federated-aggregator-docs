    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>STATIC COMPOSITION-RENDER DESIGN</span><h2>A future target composition can be assessed as disabled and non-public without creating a configuration.</h2></div><p>The private-root synthetic injection contract remains quality-gated at <code>19340fc</code>. The next boundary is now documented only: a scalar static target-composition render decision requires configuration eligibility, fresh image eligibility, prepared synthetic injection, opaque target-owned root binding, and disabled activation. It fixes no-port/no-ingress/no-restart/no-host-network controls and refuses root/environment/mount/image/package/registry/target/credential/provider/configuration text. The first possible code slice is pure policy plus a deterministic fake. No renderer, Compose-file update, image binding, package access, target action, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
