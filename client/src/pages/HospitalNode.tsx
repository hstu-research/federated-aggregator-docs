    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>LOCAL ADAPTER-PORT INTEGRATION REVIEW</span><h2>A future operation-scoped port can hide root and correlation from coordinator code.</h2></div><p>The parallel local adapter remains quality-gated at <code>ce171d8</code>. The new review defines its future narrow interface: a local composition root privately owns root, correlation, and adapter, while the coordinator may see only scalar hydrate, claim, and terminal results. The port retires after terminal closure and suppresses replay; no root, identity, record, byte, error, configuration, target, credential, or runtime detail crosses it. No port is wired, no target root injected, and no coordinator, parser, renderer, configuration, target, package/credential, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
