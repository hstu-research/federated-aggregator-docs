    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>LIVE IMAGE RESULT</span><h2>A fresh private synthetic image build succeeded with BuildKit OCI provenance requested; target staging remains a separate gate.</h2></div><p>Exactly one fresh immutable candidate completed real checkout, build platform setup, private registry authentication, build/publish, and workflow closure after the prior attestation path failed terminally. Fixed BuildKit OCI provenance was requested directly in the successful build. Package inventory cannot be listed by the available integration, so no artifact identity or visibility is publicly claimed. The image remains target-unbound: no Azure action, target pull/binding, Compose render, projection, proof, training, submission, or aggregation action occurred. The next separate gate is a real Azure target-staging dossier with target-only private package pull and fresh aggregation-disabled preflight.</p><CheckCircle2 size={24} /></section>
        </div>
