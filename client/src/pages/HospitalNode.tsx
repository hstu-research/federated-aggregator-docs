    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>LOCAL-ONLY ADAPTER INCREMENT</span><h2>The durable-store adapter implementation is authorized only for local isolated fixtures; deployment and external organization authorization remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>4c7d352</code> provides only in-memory terminal persistence. The next local-only adapter must take a constructor-owned private-root capability, reject request-derived paths, and confine filesystem work to a fixed scalar record plus bounded temporary fixtures. It may not read environment configuration, access developer homes, use Azure, query GitHub/registry/Core, bind a port, or expose paths/content/errors. Strict formatting/types/tests, corruption/permission/symlink/atomic/restart fixtures, and an import guard are required. No deployment, credential, package, target, proof, training, submission, or aggregation action is authorized.</p><CheckCircle2 size={24} /></section>
        </div>
