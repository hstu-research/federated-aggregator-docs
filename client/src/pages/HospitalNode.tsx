    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>d8857d3</i></strong></div><p>Hospital Node Quality Gates run 32619077821 completed successfully after the private file-channel adapter release: formatting, strict TypeScript, 41 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>eleven design artifacts</span></div>{documents.map((document, index) => <a key={document.file} className="node-document" href={`https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>)}</section>
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Deliver reviewed Core and Agent runtime adapters into the Azure topology before a one-shot profile can create its handoff.</h2></div><p>Agent and Core source gates are green: the Agent has a fake-tested scalar file-channel adapter and non-executing directional topology template, while the Core has a deterministic coordinator channel fake and an explicit composition factory. Azure read-only checks confirm Core live/ready health and the aggregation worker disabled marker, but the target has no Hospital Node Agent source, deployable Agent image, concrete Core coordinator runner, or composite profile to render. The proof is blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
