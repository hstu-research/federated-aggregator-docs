    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>CONFIGURATION-REFERENCE VALIDATOR — QUALITY GATED</span><h2>A pure validator now closes symbolic intent safely—without resolving configuration or reaching a root.</h2></div><p>Agent revision <code>543f172</code> adds the one-use scalar validator. It accepts one exact frozen symbolic private-root intent, privately canonicalizes only allowlisted category/binding/state, and returns frozen scalar receipts/readouts. Valid, malformed, unsupported, replay, independent-instance, freezing/redaction, nonenumerability, and import-isolation cases are covered. The full local chain passed 188 TypeScript plus 4 Python tests; remote Quality Gates run <code>32726445658</code> succeeded. This is source quality only: no configuration resolution, environment/secret/root/path/filesystem action, application/runtime wiring, target interaction, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
