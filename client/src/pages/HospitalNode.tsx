    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>TARGET-STAGING DESIGN</span><h2>The successful private synthetic image is still target-unbound; Azure staging is specified as its own non-public, inert deployment gate.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. The new dossier requires target-only private-package pull custody, redacted immutable binding, a no-port/no-ingress/no-restart composition, and fresh authoritative Core health plus aggregation-disabled preflight. Package inventory cannot be listed by the available integration, so no artifact identity or visibility is publicly claimed. No target secret, Azure configuration, target pull, Compose render, Agent start, Core call, projection, proof, training, submission, or aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
