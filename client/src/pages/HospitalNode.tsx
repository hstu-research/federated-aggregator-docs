    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>COORDINATOR COMPATIBILITY BOUNDARY</span><h2>A future fake coordinator can see scalar durability only—never root, correlation, or record state.</h2></div><p>The temporary-root factory remains quality-gated at <code>059c9c8</code>. The new review limits a future source-only coordinator fake to hydrate, claim, terminal, dispose, and snapshot operations. A fixture composition root privately constructs and injects one port; normal flow is empty hydrate → claim → deterministic no-artifact route → terminal, while terminal/orphan, denial, disposal, and replay close deterministically. Receipts never project root, correlation, record, path, factory input, or port state. No port is wired, no coordinator runtime changes, and no target-root, parser, renderer, configuration, target, package/credential, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
