    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>MACHINE-IDENTITY DESIGN</span><h2>The denied credential route is closed; a dedicated organization-owned target machine identity is designed before any second attempt.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c620747</code> retains an inert no-port/no-restart/no-network composition. GitHub’s documented Container Registry route requires classic package-read authentication, with separate organization authorization where SSO applies; the denial does not establish package access, repository linkage, inheritance, SSO, membership, or image state. The new dossier defines a distinct machine account, verified narrow package read, target-only custody, scalar diagnosis, and no fallback/retry. No second credential attempt, package/image discovery, pull, render, Agent start, Core call, proof, training, submission, or aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
