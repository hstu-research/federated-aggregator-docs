    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>CONFIGURATION INTENT SOURCE QUALITY</span><h2>Private-root configuration and fresh-image eligibility are quality-gated as pure scalar policy; target controls remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c3353e6</code> implements the versioned configuration-intent validator and deterministic fake. It evaluates only scalar mode/binding/source-release/image-binding/activation facts. Exact disabled/no-binding/no-image and fully eligible ready-not-activated states are the only disabled outcomes; missing root, unverified source, stale/mutable/unavailable image, active runtime, mismatch, unsafe disabled state, malformed observation, and replay close terminally. Local CI and Hospital Node Quality Gates succeeded. No environment/parser/filesystem/deployment/root/image/target/credential/package/proof/training/submission/aggregation action occurred. Target-owned injection remains a separate design boundary.</p><CheckCircle2 size={24} /></section>
        </div>
