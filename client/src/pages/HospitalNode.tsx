    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PROTECTED TARGET-ROOT CAPABILITY — DESIGN ONLY</span><h2>A future root may be owned by one protected composition module, never requested by a coordinator or application.</h2></div><p>The next design sets a future protected composition module as the sole resolver of one opaque configuration reference and the sole private handoff point to one local-state adapter. No root, reference, path, record, bytes, ownership/mode fact, error, configuration value, target, credential, or runtime object may cross the scalar-port boundary. Missing, malformed, non-private, shared, symlinked, unavailable, cleanup-uncertain, or replayed states close before coordinator admission with only allowlisted aggregate codes. The next possible source slice is a scalar configuration-reference validator fake—not a real root check. No root injection, filesystem/environment action, application/runtime wiring, target interaction, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
