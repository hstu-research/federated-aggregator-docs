    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>COMPOSITION CONTRACT INCREMENT</span><h2>Private-root composition is now specified as a pure scalar contract; deployment and external organization authorization remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>8ad309f</code> implements only local durable storage. The next contract accepts only allowlisted root/binding/activation/runtime-posture/rollback classes and returns terminal disabled or blocked, retry-disabled decisions. Unknown/fallback/default-activation/target-runtime/destructive-rollback/replay claims are denied before deterministic fakes, with aggregate-only readout. It cannot express a path, mount, user, permission, image, package, credential, target, process result, or listener fact. No root/configuration/mount/image/Azure/Agent/package/credential/Core/proof/training/submission/aggregation action has occurred.</p><CheckCircle2 size={24} /></section>
        </div>
