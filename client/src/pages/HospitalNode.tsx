    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>nine design artifacts</span></div>{documents.map((document, index) => <a key={document.file} className="node-document" href={`https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>)}</section>
          <section className="node-next"><div><span>NEXT FAKE-ADAPTER GATE</span><h2>Prove the typed contract locally before an Agent may open a socket.</h2></div><p>The typed hospital-node Core-client and pathless private-workspace contract is now fixed: only the guarded intent and stream routes, scalar status classes, full-body evidence, and cleanup semantics are allowed. Next, deterministic fakes may prove route/status denial and workspace compensation. No token source, Core HTTP request, Azure Agent execution, filesystem, Python training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
        <aside className="node-provenance" aria-label="Bounded Azure proof provenance">
          <span>PROOF / 005</span>
          <h2>Local verifier<br />evidence</h2>
          <StatusStamp status="VALIDATED" />
          <div><span>RELEASE</span><strong>ec217dd</strong></div>
          <div><span>AUTHORITY</span><strong>local verifier</strong></div>
          <div><span>BOUND</span><strong>generated bytes in memory</strong></div>
          <div><span>CLOSURE</span><strong>promote / clean / abort</strong></div>
          <p>Fake-first verification only. No Core connection, filesystem, real storage, training, submission, hospital data, or provider access.</p>
        </aside>
      </div>
    </div>
  );
