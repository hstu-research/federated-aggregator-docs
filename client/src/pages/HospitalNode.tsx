    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DURABLE-STORE SOURCE QUALITY</span><h2>The local durable-store adapter is quality-gated with contained root redaction; deployment and external organization authorization remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>8ad309f</code> implements an injected-private-root, fixed-record adapter with strict owner/mode/type/symlink/temporary checks, bounded canonical scalar JSON, and temporary-file/flush/atomic-promotion behavior. Corruption fixtures pass locally. An initial local assertion found the injected root enumerable in serialization; it was corrected before final CI and remote quality success. Only temporary test fixtures were touched. No environment/home, target, Azure, provider, credential, package, image, Agent runtime, Core call, proof, training, submission, or aggregation action occurred. A separate local composition/configuration design is next.</p><CheckCircle2 size={24} /></section>
        </div>
