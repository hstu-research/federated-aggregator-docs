...
const documents = [
  ...
  { title: "Public dataset metadata and eligibility review", description: "Public-page review only; all supplied data candidates remain ineligible pending independent governance.", file: "KAGGLE_BREAST_CANCER_DATASET_PUBLIC_METADATA_AND_ELIGIBILITY_REVIEW.md" },
  { title: "Independent dataset data-use governance dossier", description: "Explicit authority, rights, ethics, isolation, and analysis gates; all supplied candidates remain ineligible.", file: "KAGGLE_BREAST_CANCER_DATASET_INDEPENDENT_DATA_USE_GOVERNANCE_DOSSIER.md" },
  { title: "Authority-ready dataset decision worksheet", description: "Blank independent-review fields; it is not an approval or data-intake mechanism and all candidates remain ineligible.", file: "KAGGLE_BREAST_CANCER_DATASET_DATA_USE_DECISION_WORKSHEET.md" },
  { title: "Supervisor dataset governance handoff", description: "Brief independent-review routing note; it requests no dataset access and all candidates remain ineligible.", file: "KAGGLE_BREAST_CANCER_DATASET_SUPERVISOR_GOVERNANCE_HANDOFF.md" },
  { title: "Blank pre-empirical protocol-registration checklist", description: "Unexecuted-study planning fields only; it is not a registration, approval, data-use, or model-execution mechanism.", file: "SYNTHETIC_FEDPROX_PREEMPIRICAL_PROTOCOL_REGISTRATION_CHECKLIST.md" },
  { title: "Thesis governance appendix crosswalk", description: "Claim-safe manuscript placement guide for the governance record; it adds no data-use or empirical evidence.", file: "SYNTHETIC_FEDPROX_THESIS_GOVERNANCE_APPENDIX_CROSSWALK.md" },
  { title: "Blank study deviation-and-amendment log", description: "Change-control template for a proposed study; it records no amendment, approval, or executed action.", file: "SYNTHETIC_FEDPROX_PREEMPIRICAL_DEVIATION_AND_AMENDMENT_LOG.md" },
  { title: "One-page thesis examiner summary", description: "Assessment aid stating the source-only contribution, data-governance stop condition, and explicit non-results.", file: "SYNTHETIC_FEDPROX_THESIS_EXAMINER_SUMMARY.md" },
  { title: "Final thesis consistency checklist", description: "Editorial boundary review across the thesis package; it is not independent approval or empirical evidence.", file: "SYNTHETIC_FEDPROX_THESIS_FINAL_CONSISTENCY_CHECKLIST.md" },
  { title: "Blank thesis evidence-review register", description: "Evidence-classification template; it does not turn a record into approval, data use, or empirical proof.", file: "SYNTHETIC_FEDPROX_THESIS_EVIDENCE_REVIEW_REGISTER.md" },
  { title: "Concise thesis viva Q&A brief", description: "Oral claim-control preparation; it does not turn a response into approval, data use, or an empirical result.", file: "SYNTHETIC_FEDPROX_THESIS_VIVA_QA_BRIEF.md" },
...
const documentVolumes = [
  { start: 0, end: 9, label: "VOLUME / 10.1", title: "Hospital Node boundary", note: "Foundational product, requirements, schema, Core contract, and controlled delivery records.", range: "DOC. 01—09", state: "PROVISIONAL" as const },
  { start: 9, end: 19, label: "VOLUME / 10.2", title: "Thesis framing", note: "Evidence matrix, methods, related work, readiness, and claim-safe thesis drafting records.", range: "DOC. 10—19", state: "PROVISIONAL" as const },
  { start: 19, end: 30, label: "VOLUME / 10.3", title: "Manuscript assembly", note: "Results language, conclusion, defense, integrated manuscript, and editorial review materials.", range: "DOC. 20—30", state: "PROVISIONAL" as const },
  { start: 30, end: 37, label: "VOLUME / 10.4", title: "Thesis package", note: "Editable manuscript, review package, citation boundary, and verified front-matter records.", range: "DOC. 31—37", state: "PROVISIONAL" as const },
  { start: 37, end: 48, label: "VOLUME / 10.5", title: "Synthetic closure chain", note: "Design and local evidence for deterministic generated-marker closure only.", range: "DOC. 38—48", state: "PROVISIONAL" as const },
  { start: 48, end: documents.length, label: "VOLUME / 10.6", title: "Data-use governance", note: "Public metadata, independent governance gates, and the retained no-data-use posture.", range: "DOC. 49—60", state: "BLOCKED" as const }
] as const;
    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-documents"><div className="section-heading"><span>DOCUMENTED DELIVERY RECORD</span><span>indexed archival volumes</span></div><div className="node-document-archive">{documentVolumes.map((volume) => <section className="node-document-volume" key={volume.label}><div className="node-volume-main"><header className="node-volume-heading"><div className="node-volume-tab"><span>{volume.label}</span><small>{volume.range}</small></div><div><span>ARCHIVE SECTION</span><h2>{volume.title}</h2><p>{volume.note}</p></div></header>{documents.slice(volume.start, volume.end).map((document, offset) => { const index = volume.start + offset; return <a key={document.file} className="node-document" href={document.href ?? `https://github.com/hstu-research/federated-aggregator-docs/blob/main/docs/${document.file}`} target="_blank" rel="noreferrer"><span>{String(index + 1).padStart(2, "0")}</span><div><h2>{document.title}</h2><p>{document.description}</p></div><FileText size={19} /><ArrowUpRight size={17} /></a>; })}</div><aside className="node-volume-provenance"><span>PROVENANCE LANE</span><strong>{volume.range}</strong><StatusStamp status={volume.state} /><div><span>VOLUME</span><small>{volume.label}</small></div><div><span>MODE</span><small>document record</small></div><p>Archive grouping assists review. It does not expand the evidence boundary of any document.</p></aside></section>)}</div></section>
          <section className="node-next"><div><span>VIVA GATE — CONCISE Q&A BRIEF RETAINS ORAL CLAIM BOUNDARY</span><h2>A response can explain an absence without inventing a result.</h2></div><p>The concise viva brief prepares defensible answers on the source-only contribution, governance stop condition, local-control limits, and explicit non-results. It is not approval, data intake, an empirical script, or an Agent capability. All supplied candidates remain ineligible, the study remains unexecuted, and no authorization, data/model, clinical, privacy, deployment, or runtime evidence was added.</p><CheckCircle2 size={24} /></section>
        </div>
