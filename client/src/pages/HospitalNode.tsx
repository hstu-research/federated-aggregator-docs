    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>1982f5d</i></strong></div><p>Hospital Node Quality Gates run 32689071998 completed successfully after the authorized source-only Node secret-read edge release: formatting, production-source import guard, strict TypeScript, 74 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>The Azure target-binding decision is complete; the source release remains deliberately unbound and image, staging, and projection remain blocked.</h2></div><p>The record names only opaque binding classes and immutable admission criteria. It classifies the current Agent release as source-validated and target-unbound: no image digest, protected deployment record, target binding, projection, Agent source, Compose profile, or runner exists. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but those observations do not prove Agent platform or binding readiness. The next safe work is image-build/release-mapping design only. The proof remains blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
