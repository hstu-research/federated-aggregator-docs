    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>DIRECT THESIS PATH — LOCAL SEED IS INTEGRITY-VERIFIED, NOT TARGET-ADMISSIBLE</span><h2>The exact pnpm class is sealed locally, but provenance remains unassessed and target use is blocked.</h2></div><p>One independent scripts-disabled npm route verified exact <code>pnpm@10.16.1</code>, finite size, local integrity, and read-only self-check. Registry integrity metadata is present but not independently cross-verified, and no provenance attestation was evaluated; the resulting <code>local_seed_integrity_verified_provenance_unassessed</code> seed cannot transfer, install, activate, or build. No target, source build, container/Agent, Core/runtime, proof, data/model/trainer, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
