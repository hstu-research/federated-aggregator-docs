    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>2b0a018</i></strong></div><p>Hospital Node Quality Gates run 32694621127 completed successfully after the source-only protected-builder workflow-policy validator release: formatting, all protected import guards, strict TypeScript, 101 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Independent reviewer and custodian roles are unavailable; the Agent remains image-free, target-unbound, and blocked from provider configuration or external execution.</h2></div><p>Read-only inspection confirms repository administrator authority and a tracked container-build source, but it does not establish safe role separation. Provider plan capability is unverified, team inventory is inaccessible to the available integration, and the organization view shows one visible member with no configured teams. The dispatching owner cannot also be the independent reviewer/custodian, and no workaround identity or reduced policy will be created. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. A second externally controlled reviewer/custodian identity or independent team must be assigned before a new feasibility check; provider configuration and external execution remain blocked. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
