    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-evidence-row"><div><span>REPOSITORY / COMMIT</span><strong>hstu-research / <i>36a9c18</i></strong></div><p>Hospital Node Quality Gates run 32667220978 completed successfully after the deterministic fixed private proof-channel filesystem fake release: formatting, strict TypeScript, 59 TypeScript tests, and four Python optimization tests.</p><a className="editorial-link" href="https://github.com/hstu-research/federated-aggregator-hospital-node" target="_blank" rel="noreferrer">Inspect repository <ArrowUpRight size={15} /></a></section>
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Concrete runtime bindings are now designed; each must be implemented and quality-gated separately before Azure staging.</h2></div><p>The published binding design fixes four isolated port families: separate workload identity, closed two-operation HTTP transport, Agent-private workspace, and fixed directional handoff/result channel. Agent and Core source gates remain green, but the binding designs add no real adapter, built/released image, target Agent source, executable Core coordinator runner, or composite profile. Azure read-only checks still confirm Core live/ready health and the aggregation worker disabled marker. The proof remains blocked before fixture creation, token acquisition, lease, intent, stream, or workspace write. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
