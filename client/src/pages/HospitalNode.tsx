    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>TARGET-OWNED INJECTION DESIGN</span><h2>Private-root injection is designed as a one-way nonenumerable target capability; target controls remain blocked.</h2></div><p>The fresh immutable candidate completed real build/publish with BuildKit OCI provenance requested, while the earlier unavailable-attestation candidate remains closed. Release <code>c3353e6</code> provides source-only configuration policy. The new design permits a target composition root to hand one opaque reference only to an infrastructure injector after scalar intent is eligible/ready-not-activated. The injector validates bounded syntax, retains private nonenumerable capability, constructs the reviewed durable adapter once, and returns only a persistence port or terminal disabled class. Application/policy never receives root/path/mount/configuration. Synthetic fixtures prove redaction/one-time handoff/denial without external access. No raw reference/parser/mount/build/pull/target/credential/proof/training/submission/aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
