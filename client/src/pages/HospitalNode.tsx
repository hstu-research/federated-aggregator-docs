    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>CONCRETE STATIC-RENDERER REVIEW</span><h2>A future renderer may validate an internal inert shape, but cannot emit or activate a target configuration.</h2></div><p>The source-only render-intent contract remains quality-gated at <code>ad2e2ed</code>. The new review assigns all blueprint content and protected values to a target-owned opaque capability, usable only after one scalar <code>disabled_not_rendered</code> decision. Future temporary text must remain private, validate a fixed no-port/no-ingress/no-restart/no-host-network shape, and be discarded; promotion is forbidden until a separate deployment gate. No blueprint, renderer, file, configuration, image/root binding, package/credential access, staging, proof, training, submission, or aggregation action occurred.</p><CheckCircle2 size={24} /></section>
        </div>
