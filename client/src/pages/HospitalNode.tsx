    ...
    <div className="doc-page node-dossier">
      ...
      <div className="node-ledger-layout">
        <div className="node-ledger-main">
          ...
          <section className="node-next"><div><span>PRE-ROUTE BLOCK</span><h2>Source-only action consumption is validated; the Agent remains image-free, target-unbound, and blocked from external execution or staging.</h2></div><p>The reconciled protected-builder baseline and action-consumption contract bind only scalar admission, authorization, freshness, pre-route review, retirement, and non-executing route-placeholder states. A placeholder does not create an instruction, candidate appointment, credential, process, network route, registry contact, image action, Azure activity, target binding, Compose profile, projection, or runner. Azure read-only checks still show Core live/ready health and the aggregation worker disabled marker, but do not prove Agent image or platform readiness. Actual execution requires a separately named external provider, explicit permission and credential authority, immutable external evidence, a target-staging dossier, and a security-critical confirmation. No training, update, submission, provider contact, or aggregation-worker activation is permitted.</p><CheckCircle2 size={24} /></section>
        </div>
