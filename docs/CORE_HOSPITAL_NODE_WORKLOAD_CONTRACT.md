...
**Status:** Additive Core lease, descriptor-only read-intent, and Core-mediated generated-model stream boundaries implemented and bounded Azure proofs validated; Agent local receipt/value persistence and fake-first in-memory verification implemented locally; Agent transport/real-workspace integration, artifact update capability, submission, and training boundaries remain proposed.
