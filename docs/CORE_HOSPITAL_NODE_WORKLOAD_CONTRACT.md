...
## 35. Evidence record — deterministic typed transport and pathless workspace fakes

Hospital Node Agent commit `afc98d1` implements L4b against the already published L4 contract. The deterministic typed fake accepts only descriptor-intent issuance and model-stream opening with assignment/read-intent IDs and idempotency key. It enforces intent-before-stream sequencing, projects only scalar receipt/stream facts plus an async generated-body iterator, and returns allowlisted terminal or retryable codes. It accepts no URL, header map, token, Range, provider field, arbitrary route, raw response, storage capability, or network configuration.

The matching private-workspace fake is in-memory and pathless. It keeps generated bytes internally, exposes only aggregate lifecycle counts, permits promotion only after exact checksum/byte-size equality, and requires cleanup after a mismatch or interrupted generated body. It has no filesystem root/path, key, provider, token, socket, Core request, Azure reference, trainer, update, submission, or aggregation dependency.

Local quality passed with formatting, strict TypeScript, 25 TypeScript tests, and 4 Python tests. This is fake compatibility evidence only; it does not establish actual OIDC use, Core transport, private filesystem security, provider isolation at runtime, Agent deployment, Azure execution, model release, training, update submission, aggregation, hospital integration, real data use, or clinical validity. The next gate is L4c concrete-adapter review, which remains design-only and cannot run a real request.

## 36. Design record — concrete Agent transport and private-workspace review

L4c fixes the prerequisites for later concrete Agent adapter code without writing or invoking it. Configuration remains composition-root owned: a normalized fixed HTTPS Core origin, literal hospital-node audience, safe bounded time/size values, and a dedicated private workspace root. The concrete transport will own the workload token and exactly two guarded Core route constructors; it will have no generic request method, caller-provided endpoint/header map, provider capability, direct storage dependency, redirect following, Range behavior, raw response projection, automatic retry, or identity reuse.
