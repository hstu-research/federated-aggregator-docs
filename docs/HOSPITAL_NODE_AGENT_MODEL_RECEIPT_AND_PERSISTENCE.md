...
**Status:** L2 scalar-safe receipt persistence and L3 fake-first local verification implemented and tested; no Agent Core client, token flow, HTTP stream, real/private filesystem workspace, storage-provider contact, training, update, submission, aggregation, or Azure Agent execution is implemented by this document.
...
## 10. Implementation evidence — L2 local receipt contracts and SQLite persistence

Hospital Node Agent commit `a9561ff` implements the first two local delivery substeps without connecting the Agent to Core. `packages/contracts` now contains `hospital-node-base-model-read-receipt/v1`, a strict scalar-safe receipt validator, and an equally narrow observed-materialization validator. The accepted receipt has only assignment/read-intent IDs, immutable command and descriptor digests, checksum, positive byte size, the Core allowlisted `application/vnd.fedagg.base-model+zip` content type, expiry, and `issued` state. Any unknown field is rejected; this blocks token-, URL-, locator/version-, provider response-, raw header/body-, workspace-path-, byte-, local-data-, and free-text diagnostic-shaped projections at the contract boundary.

`packages/domain` adds an immutable local receipt state matrix. A receipt can transition from `issued` only to terminal `verified`, `rejected`, `aborted`, or `expired`; a materialization assessment requires exact receipt ID, checksum, byte-size, and content-type equality. L2 has no `streaming` state because no Core HTTP response or byte body is opened in this slice. The state model is a pure local value boundary and does not import token, transport, filesystem, SQL, trainer, or storage code.

`SqliteBaseModelReceiptRepository` adds the documented local records: `base_model_read_receipts`, `local_model_materializations`, and append-only `model_receipt_events`. It persists immutable receipt facts and an observed terminal scalar outcome/event inside one SQLite transaction; exact issue replay is idempotent and completed `verified` or `rejected` records are terminal after a process restart. SQLite contains no byte column, workspace path, provider metadata, locator, header, token, secret, local-data field, or free-text diagnostic. The current local table shape uses the receipt ID as the materialization key and relies on SQLite’s append-only sequence for events; it does not claim an independent path-bearing materialization identity.

Local `pnpm run ci` passed after the commit: formatting, strict TypeScript, **17 TypeScript tests**, and **4 Python tests**. The tests cover valid and expired receipts, unexpected locator/path-shaped fields, exact/mismatched observed facts, terminal transition denial, idempotent receipt issue, atomic event evidence, and restart-safe terminal rejection. Node’s SQLite experimental-runtime warning remains an inherited runtime-support limitation, not a production-readiness assertion. No Core route was called; no OIDC token was acquired; no generated fixture bytes, HTTP header, model workspace, local path, training, update, submission, aggregation, hospital data, or Azure Agent resource was used.

The next permitted boundary is **L3** only: a local verification use case and fake generated-fixture workspace adapter. It must remain fake-first and local, compute integrity over generated bytes, prove cleanup/redaction/restart behavior, and stop before any real Core client, workload token, HTTP stream, Azure Agent execution, Python training, update/submission, or aggregation behavior.

## 11. Implementation evidence — L3 fake-first local receipt verification

Hospital Node Agent commit `ec217dd` implements L3 as a deliberately offline verification exercise. `verifyBaseModelReceiptLocally` takes an already persisted local receipt, generated `Uint8Array` chunks, an allowlisted content type, the L2 repository port, and a workspace port. It incrementally computes SHA-256 and byte count, compares only scalar observed facts with the receipt, and returns only receipt ID, terminal result class, and three equality booleans. It does not return a body, temporary handle, promoted fixture, local path, URL, header, token, credential, provider detail, or model payload.

The only workspace implementation is `InMemoryGeneratedFixtureWorkspace`, a test adapter with temporary, promoted, cleaned, and discarded lifecycles held in process memory. Its safe test projection contains aggregate lifecycle counters only. For an exact generated fixture, the use case promotes the temporary fixture before asking L2 to persist `verified`; if terminal persistence cannot complete, it discards the promoted in-memory fixture and fails closed. A mismatch persists `rejected` then cleans the temporary fixture. An interrupted generated body cleans the temporary fixture and records `aborted`. A receipt already terminal at restart is skipped before the workspace opens. These are compensation rules for local fake state, not a cross-resource transaction, durable filesystem design, or production recovery claim.
...
The next permitted slice is **L4**, specified in `HOSPITAL_NODE_AGENT_CORE_CLIENT_AND_PRIVATE_WORKSPACE.md`. L4b deterministic fakes now provide local compatibility evidence only. The next step is L4c adapter review before any concrete typed Hospital Node Core client or private workspace code: it must retain the separate hospital-node identity, call only the existing descriptor-intent and guarded stream routes, deny redirects/range responses/raw body logging, and remain strictly before any Azure Agent execution, training, update, submission, or aggregation proof.
