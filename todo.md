...
- [ ] Safety gate L4e2x — Before any protected image build/push, publish a separate builder authorization record naming the external builder owner, registry approval, scoped credential custody, approved base identity, build-environment containment, build-log redaction, candidate quarantine/rollback owner, and no-target-deployment attestation. Without it, no Docker/remote builder run, registry push, Azure staging, Compose render, projection open, or proof may proceed.
