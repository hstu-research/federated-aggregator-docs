...
- [ ] Increment L4e2z3 — Design a source-only one-candidate execution-authorization envelope contract with strict scalar facts, deterministic independent review fakes, freshness/expiry/revocation/quarantine/post-route-placeholder closure, aggregate-only readout, and forbidden-import tests. Do not create an execution instruction or access any external builder, registry, Docker, credential, Azure, target, projection, Compose, runner, proof, training, submission, or aggregation capability.
