...
- [ ] Increment L4e2k1 — Define the concrete token-source binding contract, including protected secret-reference ownership, literal audience, bounded token lifetime, zero token persistence/output, and deterministic denial fixtures. Do not read a secret or acquire a token.
- [ ] Increment L4e2k2 — Define the fixed HTTP request-port binding contract, including an internally fixed HTTPS Core origin, connection/response limits, no redirect/Range/encoding fallback, scalar-safe error mapping, and deterministic response fixtures. Do not import or invoke fetch.
- [ ] Increment L4e2k3 — Define the private filesystem and handoff/result channel binding contracts, including fixed tmpfs roots, restrictive ownership/modes, exact record names, directional access, exclusive writes, cleanup, and crash/close behavior. Do not use a host filesystem, enumerate a directory, or render a Compose profile.
