...
- [ ] Increment L4e2l — Compose all four separately validated fake port families through the existing injected Agent runtime composition factory and one-shot runner, then prove one complete scalar lifecycle, terminal failures, redaction, no retry, and cleanup with no real secret, transport, filesystem, channel, image, Azure, or proof capability.
