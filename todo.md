...
- [ ] Increment L4e3x4 — Run the documented Azure target-staging sequence: publish target-safe composition source and policy tests; pass Agent quality/release gates; perform only a fresh scalar Azure/Core preflight; configure one target-only least-privilege package pull secret; stage exactly one immutable synthetic candidate without a public port; then publish redacted scalar staging closure. Proof remains `not_invoked`.
