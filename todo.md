...
- [ ] Increment L4e3x5 — The package-access authorization diagnosis and dedicated target machine-identity dossier is now published. Next, implement source-only scalar diagnostic contracts and quality tests; then perform only scalar read-only package/authorization checks. Classify the denied route without package/image discovery and retain the new identity design’s prohibition on revoked or existing human/browser/repository/Core/worker/callback credential reuse. No image pull, target render, Agent start, proof, or aggregation action is permitted.
