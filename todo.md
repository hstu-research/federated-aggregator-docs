...
- [ ] Safety gate L4e2z — Before any external protected-builder execution, publish a documentation-only execution-authorization decision defining the one-candidate authorization envelope, independent approval evidence freshness, credential-custody readiness preconditions, immutable source/base re-verification, explicit external invocation stop rules, post-route failure publication, and no-target-deployment re-attestation. Do not create/use a credential or contact a builder, registry, Azure, target, or projection.
