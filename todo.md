...
- [ ] Increment L4e3x5k — Create a focused static target-composition render design before any Compose file, mount, image binding, target configuration, or runtime action. Specify scalar render input, fixed no-port/no-ingress/no-restart constraints, target-owned private-root binding isolation, no-path redaction, disabled default, image eligibility mapping, forbidden render fields, static fixture matrix, rollback, and staging/proof preconditions. No renderer implementation or target action is authorized in that design increment.
