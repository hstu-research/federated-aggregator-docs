...
- [ ] Safety gate L4e2v — Before an image build/push implementation, publish a separate protected image-build and release-mapping decision. It must define the build owner, non-root/no-runtime-default constraints, reproducible source/dependency admission, immutable image digest record, registry access boundary, build-log redaction, rollback/quarantine, and absence of target deployment. Without it, no image build/push, Azure staging, Compose render, projection open, or proof may proceed.
