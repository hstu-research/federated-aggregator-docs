...
- [ ] Increment L4e2j — Publish and implement an injected-only Agent runtime adapter composition factory using the already reviewed typed Core client, pathless workspace, private channel, and one-shot runner. It must require explicit enablement and prevalidated config, use deterministic fake ports only, return scalar-safe results, and add no OIDC/fetch/filesystem/channel discovery/image binding/target runtime, fixture, training, submission, or aggregation behavior.
