...
- [ ] Increment L4e3x5d — Implement the separately authorized local-only durable-store adapter and isolated temporary-fixture coverage. Enforce fixed private-root ownership, canonical scalar encoding, bounded load, symlink/type/permission rejection, atomic promotion, corruption closure, and aggregate-only diagnostics. Confine filesystem imports to the infrastructure adapter and fixtures. No deployment configuration, target action, credential, package, proof, or aggregation capability may be invoked.
