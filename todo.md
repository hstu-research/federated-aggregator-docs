...
- [ ] Increment L4e3x5j — Implement source-only synthetic private-root injection contracts and nonenumerability fixtures. Use only synthetic non-path sentinels; require scalar eligible/ready intent, one-time handoff, private nonenumerable storage, aggregate-only result, denial-before-adapter on malformed sentinel, construction failure closure, fallback/replay denial, and forbidden-import guards. Run Agent local/remote quality gates; no raw root, parser, mount, target, build, pull, credential, proof, or aggregation action may occur.
