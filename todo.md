...
- [ ] Increment L4e2o — Publish a design-only protected concrete secret-read edge dossier with fixed projection ownership, exact kind/mode/owner checks, bounded nonpersistent buffer lifecycle, scalar safe-error map, deletion/rotation/restart semantics, fake-first implementation slices, and no Node filesystem import outside that later edge. Do not open a projection, bind an image, stage Azure, render Compose, or invoke a proof in the design slice.
