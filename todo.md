...
- [ ] Increment L4e3x5i — Create a focused target-owned private-root injection-adapter design before any raw root reference, environment/configuration parser, mount, or target change. Specify opaque reference custody, constructor/nonenumerability boundary, validator handoff, no-path/error redaction, disabled/default posture, root ownership/permission assertions, local compatibility fixtures, static composition rendering, rollback, release-image mapping, and staging/proof preconditions. No adapter implementation, image build, package pull, or target action is authorized in that design increment.
