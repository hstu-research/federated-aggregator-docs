...
- [ ] Increment L4e2i — Publish and implement a separate protected Agent proof-runner image source and non-invoking entrypoint. It may validate existing protected configuration and emit only a versioned scalar ready/refused marker; it must not build/push/deploy, acquire a token, construct a Core client/channel/workspace, read/write handoff/result, invoke the proof runner, expose a port, or invoke a target proof.
