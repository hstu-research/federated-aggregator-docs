...
- [ ] Increment L4e3x5b — The additive scalar persistence/restart boundary is published. Next, implement the pure persistence port/fake and terminal record validator; preserve restart-wide replay suppression/no retry, reject identifier/free-text/path/provider-shaped persistence, and keep all provider, credential, package, target, proof, and aggregation capabilities out of scope.
