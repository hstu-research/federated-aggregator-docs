...
- [ ] Safety gate L4e2y — Before any protected-builder execution, publish a documentation-only external execution and credential-custody decision that names the approving control plane, non-delegable credential creation/rotation/revocation authority, isolated builder location/class, registry release channel, immutable base/source pin verification, candidate deletion/quarantine custody, execution audit retention/redaction, and no-target-deployment re-attestation. Do not create/use a credential or contact a builder/registry/Azure.
