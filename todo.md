...
- [ ] Increment L4e3x5g — Create a focused concrete composition/configuration adapter design before any environment, mount, Docker/Compose, or target change. Specify target-only root reference ownership, fixed configuration schema, parser/validator, default-disabled behavior, no-root/no-fallback denial, no-path redaction, static composition render checks, rollback, compatibility fixtures, release/image mapping, and staging/proof preconditions. No adapter implementation or target action is authorized in that design increment.
