...
- [ ] Increment L4e2q — Publish a separate design-only concrete Node filesystem secret-read edge review. Fix fixed protected binding ownership, non-following open/metadata/read/close semantics, exact platform policy abstraction, bounded buffer/disposal behavior, scalar error translation, no general filesystem capability, and a separate fake-first concrete-edge implementation plan. Do not import Node filesystem APIs, open a projection, bind an image, stage Azure, render Compose, or invoke a proof in the design slice.
