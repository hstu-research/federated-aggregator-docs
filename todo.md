...
- [ ] Increment L4e3x4 — Run the documented Azure target-staging sequence: an initial scalar Azure/Core preflight has confirmed Core liveness/readiness, resolved disabled aggregation, and no Agent candidate. Target-safe static composition source/policy tests are now release `c620747` with local CI (106 TypeScript/4 Python) and Hospital Node Quality Gates `32697410462` passed. A presence-only target check found no private-registry credential, so staging stopped pre-route with no pull or target change. Owner authorization now permits provisioning one **new target-only** least-privilege package pull credential, but forbids user/browser/repository-operator, Core, ML-worker, callback, public, or package-wide credential reuse. Next, install that target-only credential; repeat fresh scalar preflight; stage exactly one immutable synthetic candidate without a public port; then publish redacted scalar staging closure. Proof remains `not_invoked`.
