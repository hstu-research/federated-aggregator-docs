...
- [ ] Safety gate L4e3j — Before provider configuration, publish an evidence-only protected-environment feasibility record that verifies feature availability, private package access policy, repository administrator custody, reviewer separation, no-bypass eligibility, and no implicit-environment creation risk. Do not create an environment, workflow, package, credential, image, registry route, Azure target, projection, or proof path.
