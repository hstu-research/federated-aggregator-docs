...
- [ ] Increment L4e2r — Implement a deterministic fake `NodeProtectedProjectionSyscallPort` that scripts non-following open, descriptor metadata, bounded read, post-read metadata, and close through opaque handles. Exercise unsupported-platform, open/metadata/kind/access/size/change/short-read/close/disposal denials, scalar collapse, redaction, no cache/watch/retry, and exact closure without importing Node filesystem APIs.
