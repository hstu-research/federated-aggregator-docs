...
- [ ] Increment L4e3c — Design a source-only one-candidate action-consumption state contract with strict scalar facts, deterministic pre-route/review/retirement fakes, route-not-started and non-executing route-started placeholders, terminal pre/post-route closure, aggregate-only readout, and forbidden-import tests. Do not create an instruction/capability or access any external credential, builder, registry, Docker, image, Azure, target, Compose, projection, runner, proof, training, submission, or aggregation capability.
