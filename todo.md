...
- [ ] Increment L4b — Implement only deterministic fake typed Core-client and fake private-workspace adapters against the L4 contract. Tests must cover allowed call sequencing, route/status denial, full-body fact projection, no provider fields, no raw headers/bodies/tokens/paths, and workspace cleanup compensation without opening a real socket, token source, provider client, filesystem, Azure resource, trainer, update, submission, or aggregation worker.
