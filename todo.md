...
- [ ] Increment L4e3x4 — Run the documented Azure target-staging sequence: an initial scalar Azure/Core preflight has confirmed Core liveness/readiness, resolved disabled aggregation, and no Agent candidate. Next, publish target-safe composition source and policy tests; pass Agent quality/release gates; repeat a fresh scalar preflight; configure one target-only least-privilege package pull secret; stage exactly one immutable synthetic candidate without a public port; then publish redacted scalar staging closure. Proof remains `not_invoked`.
