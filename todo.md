...
- [ ] Increment L4e3x5l — Implement only the source-only static target-composition render-intent/decision policy and deterministic fake tests. Accept versioned scalar labels only; require disabled/not-rendered posture, exact no-public/no-restart/no-host-network constraints, fresh eligibility, prepared synthetic injection, and reserved opaque binding. Cover unknown/forbidden/missing/fallback/activation-enabled/exposure/restart/host-network/sink-failure/replay closure and import guards. Do not render or write configuration, parse environment, bind an image/root, invoke Docker/Compose, access package/credential/target/Azure/Core, or enable proof/training/submission/aggregation.
