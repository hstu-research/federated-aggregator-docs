...
- [ ] Increment L4e3x5h — Implement source-only private-root configuration intent parsing and fresh-image eligibility denial contracts. Accept only allowlisted scalar mode/binding/source-release/image-binding/activation classes; deny malformed, default-enabled, missing-root, stale/mutable/unverified image, active runtime, fallback, and replay states before a fake; project aggregate-only readout; prohibit environment, filesystem, Docker/Compose, provider, credential, package, target, proof, and aggregation imports. Run Agent local/remote quality gates; no parser injection, configuration, build, pull, or target action may occur.
