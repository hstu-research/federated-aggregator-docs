...
- [ ] Safety gate L4e2u — Before image binding or Azure target staging, publish a separate target-binding decision that names the protected deployment binding mechanism/owner, platform support evidence, release-image mapping, runtime rollback containment, and no-target-invocation verification plan. Without it, no image build, Agent source staging, Compose render, projection open, or proof may proceed.
