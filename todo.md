...
- [ ] Increment L4e3x5e — Create a focused local composition/configuration design for supplying a prevalidated private-root capability to the durable-store adapter. Define configuration ownership, root provisioning, user/permission assertions, activation mode, status/log redaction, compatibility fixtures, rollback, and target/proof preconditions. No composition code, deployment configuration, target activation, or package/credential action is authorized in that design increment.
