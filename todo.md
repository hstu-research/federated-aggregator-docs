...
- [ ] Increment L2 — Implement only the documented Agent receipt/value contracts and local SQLite receipt/event persistence. No Core HTTP adapter, workspace materialization, token flow, training, update, submission, aggregation, or Azure Agent proof may occur before the preceding delivery slice is recorded.
- [ ] Increment L3 — After L2 evidence, implement the document-defined local verification use case and fake generated-fixture workspace adapter with integrity, cleanup, redaction, and restart tests. Stop before any real Core client or Azure request.
