...
- [ ] Safety gate L4e3 — Before any external protected-builder execution action, publish a documentation-only action decision defining the precise one-candidate authorization consumption rule, independent pre-route freshness recheck, credential-custody handoff prohibition, external-route declaration, post-route closure evidence, candidate retirement authority, and renewed no-target-deployment attestation. Do not create/use a credential or contact a builder, registry, Docker, Azure, target, or projection.
